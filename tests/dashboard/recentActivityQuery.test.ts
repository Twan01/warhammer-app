...
describe("getRecentActivity — SQL shape", () => {
  it("issues two parallel SELECTs (sessions JOIN units, battle_logs)", async () => {
  ...
  it("sessions query JOINs painting_sessions to units and selects session_date, id, unit_name, unit_id", async () => {
  ...
  it("battles query selects id, created_at, opponent_faction, result with LIMIT 20", async () => {
    selectMock.mockResolvedValueOnce([]);
    selectMock.mockResolvedValueOnce([]);
    await getRecentActivity();
  ...
  it("returns { sessions, battles } object with rows from each query", async () => {
  ...
  it("queries run in parallel via Promise.all (timing assertion)", async () => {
