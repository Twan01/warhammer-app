...
**Timeline:** 2026-06-11 → 2026-06-12 (1 day)
**Stats:** 111 commits, 232 files changed, +15,471 / -1,118 lines, 41/41 requirements satisfied, Nyquist 4/4 compliant

**Key accomplishments:**

- Critical fixes & dead-end elimination: Painting Mode completion-screen exit button + Escape hint and `returnTo`-aware "assignment not found" recovery, Recipes distinct error state (not empty list), Army List notes toast fires only on real mutation, deduplicated goal-delete error toast, success toasts for enhancement/leader mutations, rules-favorites optimistic rollback error toasts, dark-only token light-mode fallbacks, custom dark-zinc scrollbar
- Visual consistency pass: PageHeader (text-3xl + border-b + subtitle) adopted on Rules Hub & Unit Database and all remaining pages, standardized section heading hierarchy across Dashboard/Goals/Spending/Data Health, normalized spacing (p-6 gap-6, flex-col gap-6), icon-pill filtered empty states, status dots and button icons use theme tokens (no hardcoded hex, h-4 w-4 + mr-2)
- Feedback hardening & form UX: "Deleting…" pending text on 4 delete dialogs (Faction/BattleLog/Recipe/Paint), GameDayPage error state with retry, all Sheet forms autoFocus first input, RuleNoteEditor subtle "Saved" indicator, PlaybookTab disabled-save tooltip + error Retry button, JournalTab/snapshot success toasts, gcTime alignment for staleTime:Infinity hooks
- Navigation, cross-links & cleanup: Painting Mode exit returns to originating page (returnTo + resolveReturnTo across 5 entry points), Collection→Unit Database "View Datasheet" deep-link, bidirectional Rules Hub ⇄ Unit Database cross-links, Game Day sidebar highlight, Battle Log→army list cross-link, collapsed sidebar group dividers, sidebar collapse CSS transition
- Technical cleanup: ArmyListDetailSheet dead code removed (~340 lines), RecipeCard wrapped in React.memo, ArmyListDetailPage reducer extracted to its own file

**Tech debt accepted:** FIX-02 / NAV-01 fixed during audit remediation; 129-02 SUMMARY mis-describes header markup as "inline h1" (code correctly uses PageHeader — documentation inaccuracy only); 0126-VERIFICATION.md absent (coverage provided via VALIDATION.md Nyquist + integration check)

**Archived:**

- Roadmap: `.planning/milestones/v0.5.2-ROADMAP.md`
- Requirements: `.planning/milestones/v0.5.2-REQUIREMENTS.md`
- Audit: `.planning/milestones/v0.5.2-MILESTONE-AUDIT.md`
