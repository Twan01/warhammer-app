...
---
*Last updated: 2026-06-18 — Phase 138 (Player-Journey Depth) complete; PLAY-01/PLAY-04/PLAY-05 validated (human UAT approved). Side-by-side unit comparison on `/unit-database/compare` (batched query, shared WeaponTable, binary diff highlight, cap-3 selection); bidirectional Collection ⇆ Unit Database loop (owned-badge deep link + cross-faction search ownership + invalidation symmetry); dashboard goal-progress widget reusing the existing rules.db-free derivation. No schema change. Two code-review Criticals fixed in-flight. Theme C nearly done; next Phase 139 (Data Quality at Scale — pipeline FK/orphan validation, 22-faction audit, French translations) closes milestone v0.6.0.*
