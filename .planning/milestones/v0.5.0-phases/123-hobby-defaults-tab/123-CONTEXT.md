...
- **D-03:** A utility function `getBucketLabel(bucket: PipelineBucket, settings: AppSettingsMap): string` resolves display labels for the 5 pipeline buckets. Falls back to the default bucket name when no custom label exists. (Refined from initial `getStageLabel` per D-04 scope — only bucket labels are customizable, not individual statuses.)
...
- **D-05:** The only bucket-label consumer is `HobbyPipeline` — it reads `useAppSettings()` and passes through the label resolver. `UnitFilters`, `KanbanBoard`, and `StatusPopover` display individual `PaintingStatus` values (the 11 statuses), not the 5 bucket labels, so they do NOT need the label resolver. No new context provider needed — React Query cache already makes settings globally available. (Refined: ROADMAP SC-1 "Collection filters and Kanban columns" refers to the bucket-level pipeline display on the Dashboard, not to the individual-status UI elements.)
